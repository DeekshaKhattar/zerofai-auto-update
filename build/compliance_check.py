...
def call_complaince_parameters():
    ...
    if response.json().get('total_count'):
        for parameter in response.json()['results']:
            try:
                if parameter['type'] == 'python':
                    ...
                    complaince_check_data[parameter['parameter_name']] = run_python_command(command)
                else:
        ...
        cleaned_data = clean_and_convert(complaince_check_data)
        payload = json.dumps({
            "hostname":  socket.gethostname(),
            "complaince_data" : cleaned_data
        })
...
url=os.getenv("URL")
get_params_api_url = url+"/portal/api/v1/complaince/configuration/?page_size=1000"
post_params_api_url = url+"/portal/api/v1/complaince/"
